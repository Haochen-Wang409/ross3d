...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        self.embed_tokens = nn.Embedding(152064, config.hidden_size, self.padding_idx)
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is None or (isinstance(past_key_values, DynamicCache) and len(past_key_values.key_cache) == 0):
            first_step_flag = True

        if inputs_embeds is not None and first_step_flag:
