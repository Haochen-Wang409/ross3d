...
from ross3d.video_utils import VideoProcessor
...
def main(args):
    ...
    for k in iou25_f1_per_type.keys():
        print("===============================================")
        print(f'{k} IoU25: {np.mean(iou25_f1_per_type[k]) * 100}')
        print(f'{k} IoU50: {np.mean(iou50_f1_per_type[k]) * 100}')

        results[f'{k} IoU25'] = np.mean(iou25_f1_per_type[k] * 100)
        results[f'{k} IoU50'] = np.mean(iou50_f1_per_type[k] * 100)
...
if __name__=="__main__":
    ...
    parser.add_argument("--output-file", type=str, default='results/multi3drefer/val.csv')
